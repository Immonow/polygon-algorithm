...
class AlgorithmUnionTest extends TestCase
{
    ...
    public function testSimpleCase(): void
    {
        ...
        $this->assertEquals(
            count($tested),
            count($expected),
            sprintf('Result multipolygon should contain one polygon, but contains %d', count($tested))
        );

        // one polygon is expected in this union
        $expected_size = count($expected[0]);
        $tested_size = count($tested[0]);
     ...
     */
    public function testSimpleCaseWithHole(): void
    {
        ...
        $this->assertEquals(
            count($tested),
            count($expected),
            sprintf('Result multipolygon should contain two polygons, but contains %d', count($tested))
        );
